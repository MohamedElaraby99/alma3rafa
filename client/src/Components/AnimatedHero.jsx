...
const AnimatedHero = ({ onGetStarted }) => {
  ...
  const stats = [
    { icon: FaUsers, number: "5K+", label: "طلاب مسجلين", color: "text-green-600" },
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-green-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900" dir="rtl">
      {/* Animated Background Elements */}
      <div className="absolute inset-0">
        {/* Large green Shape - Main Background Element */}
        <div className="absolute top-0 left-0 w-[400px] h-[400px] md:w-[600px] md:h-[600px] lg:w-[800px] lg:h-[800px] bg-gradient-to-br from-green-600 via-cyan-500 to-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"></div>
        
        {/* Secondary green Shapes */}
        <div className="absolute top-20 right-20 w-48 h-48 md:w-96 md:h-96 bg-green-400 rounded-full mix-blend-multiply filter blur-2xl opacity-30 animate-blob"></div>
        ...
        <div className="absolute top-1/4 right-1/4 animate-float">
          <div className="w-4 h-4 md:w-6 md:h-6 bg-green-500 rounded-full opacity-40"></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className={`order-2 lg:order-1 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            <div className="space-y-4 md:space-y-6 text-right">
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full text-xs md:text-sm font-medium font-sans">
                🧪
              ...
              <h1 className="text-3xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-gray-900 dark:text-white leading-tight font-sans">
                <span className="text-green-600">تعلم مع مستر</span>
                <br />
                <span className="bg-gradient-to-r from-green-600 via-cyan-500 to-teal-500 bg-clip-text text-transparent">
                   أحمد الشافعي الكيمياء
                </span>
                <br />
              ...
              {user?.fullName ? (
                <a
                  href="/courses"
                  className="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-600 to-teal-500 hover:from-green-700 hover:to-teal-600 text-white rounded-lg font-bold text-lg shadow-lg transition-all duration-300 transform hover:scale-105"
                >
                  ابدأ التعلم
                </a>
              ) : (
          ...
          {/* Left Side - Image with green Shape Effect (RTL) */}
          <div className={`order-1 lg:order-2 relative transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            ...
            <div className="relative w-full h-[400px] md:h-[500px] lg:h-[600px] flex items-center justify-center">
              {/* Large green Circle Frame - Ring shape with transparent center */}
              <div className="absolute w-[300px] h-[300px] md:w-[400px] md:h-[400px] lg:w-[500px] lg:h-[500px] border-8 border-green-600 rounded-full shadow-2xl animate-pulse z-20 bg-transparent"></div>

              {/* Image Container - Positioned in the center */}
              <div className="absolute inset-0 flex items-center justify-center z-0">
                <div className="w-[280px] h-[280px] md:w-[360px] md:h-[360px] lg:w-[460px] lg:h-[460px] rounded-full border-4 border-green-600 shadow-2xl overflow-hidden">
                  <img
                    src={mr} 
                    alt="مستر أحمد الشافعي - مدرس في الكيمياء" 
                ...
                <div className="absolute -bottom-2 -right-2 md:-bottom-4 md:-right-4 w-3 h-3 md:w-4 md:h-4 bg-green-400 rounded-full animate-pulse z-30"></div>
      ...
      <div className="absolute bottom-10 right-10 animate-float">
        <div className="w-3 h-3 md:w-4 md:h-4 bg-green-500 rounded-full opacity-30"></div>
      </div>
